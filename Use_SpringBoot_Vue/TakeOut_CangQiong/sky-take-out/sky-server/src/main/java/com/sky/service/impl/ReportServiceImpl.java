...
public class ReportServiceImpl implements ReportService {
    ...
    /**
     * 统计营业额数据GSF：一次查询所有结果并返回处理
     * @param begin
     * @param end
     * @return
     */
    public TurnoverReportVO getTurnoverStatisticsGsf(LocalDate begin, LocalDate end) {
        // 获取时间范围的最开始和最结尾
        ...
        TreeMap<LocalDate, Double> turnoverMap = new TreeMap<>(comparator);

        List<LocalDate> dateList = new ArrayList<>();
        dateList.add(begin);
        turnoverMap.put(begin, 0.0);
        while (!begin.equals(end)){
            begin = begin.plusDays(1);
            dateList.add(begin);
            turnoverMap.put(begin, 0.0);
        ...
        // 查询时间范围内所有的订单数据
        List<Orders> ordersList = ordersMapper.getOrderByTimeInterval(beginTime, endTime);
        ...
        for (Orders orders : ordersList) {
            ...
            turnoverMap.put(orderDate, turnoverMap.get(orderDate) + amount);
        }
        ArrayList<Double> turnoverList = new ArrayList<>(turnoverMap.values());
